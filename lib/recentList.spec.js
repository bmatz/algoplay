...
const {addToList} = require('./recentList');
// addToList handles a list of max 5 items
// with addToList(list, item) the item is added at the top of the list, if it already exists (identified by id) it will also be moved to the top
// an list will be an object with the items indexed by their id
// to handle a correct order in an index based object of items it is necessary to save the current position value in the added items (property order)
...
describe('recent list', () => {
    ...
    it.only('adds item on top even if other items exist', () => {
        const result = addToList({}, item2);
        const result2 = addToList(result, item);
        const result3 = addToList(result2, item3);
        expect(result2).toEqual({
            123: {
                id: 123,
                name: 'foobar',
